...
class CycleGAN():
    ...
    def train(self, dataset_name, epochs, batch_size=1, train_N="normal", train_P="severe_proliferate", print_interval=100,
    ...
    def sample_images(self, epoch, batch_i, testN, testP):
        ...
        classification = [['normal', 'severe_proliferate'][int(np.argmax(self.classifier.predict(x)))] for x in imgs]

        gen_imgs = np.concatenate(imgs)
        correct_classification = ['normal', 'severe_proliferate', 'normal', 'severe_proliferate', 'normal', 'severe_proliferate']
    ...
    def predict(self, original_in_path, translated_out_path, reconstructed_out_path, force_original_aspect_ratio=False):
        ...
        if int(np.argmax(pred_original)) == 0:
            ...
            print("PREDICTION -- severe_proliferate")
...
if __name__ == '__main__':
    ...
    # gan.train(dataset_name=os.path.join("..","cyclegan_data"), epochs=1, batch_size=4, print_interval=10,
    #       sample_interval=100)
